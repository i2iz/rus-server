...
@Repository
public interface RoutineCollectionMapperRepository
        extends JpaRepository<RoutineCollectionMapper, RoutineCollectionMapperId> {
    ...
    /**
     * 특정 컬렉션 ID에 해당하는 모든 매퍼 정보를 관련된 Routine, Category 정보와 함께 조회합니다.
     * N+1 문제를 방지하기 위해 JOIN FETCH를 사용합니다.
     * 
     * @param collectionId 컬렉션 ID
     * @return 특정 컬렉션의 상세 정보가 포함된 RoutineCollectionMapper 리스트
     */
    @Query("SELECT m FROM RoutineCollectionMapper m " +
            "JOIN FETCH m.routine r " +
            "JOIN FETCH r.category " +
            "WHERE m.routineCollection.collectionId = :collectionId")
    List<RoutineCollectionMapper> findByCollectionIdWithDetails(@Param("collectionId") Integer collectionId);
}
